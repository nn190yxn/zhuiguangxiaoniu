...
- [ ] 7. 实现受控 Markdown、图片和来源展示
  - 复用安全文本格式化原则，将标题、列表、表格、引用、链接和图片转换为固定安全 HTML。
  - 禁止原始 HTML、脚本、事件属性、危险协议、任意本地路径和未授权外链图片。
  - 图片进入非脚本执行资源目录，建立 source_images 到受控资源 ID/URL 的映射；缺图只展示来源说明。
  - 后台与员工端使用同一内容安全契约，避免后台可编辑内容绕过员工端过滤。
  - [ ] 7.1 编写 XSS/URL/路径安全测试：正文、来源、标题、图片 alt、表格和链接全部覆盖恶意输入。
  - [ ] 7.2 编写渲染兼容测试：正常 Markdown、畸形表格、嵌套列表、中文标点和缺失媒体稳定降级。
