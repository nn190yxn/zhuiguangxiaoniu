...
function workloadSeedDefaults(PDO $pdo): void {
    $metrics = [
        ...
        ['sales_referral','转介绍','sales','daily_input','process','count',0,82],
        ...
        ['coach_referral_visit','邀约诺访','coach','daily_input','process','count',0,45],
        ['coach_referral','转介绍','coach','daily_input','result','count',0,65],
    ];
...
function workloadEnsureAuditRules(PDO $pdo): void {
    ...
    $stmt->execute(['sales_wechat_reach', 'sales', 1, 1, 3, 'full']);
    $stmt->execute(['sales_plan_visit', 'sales', 1, 1, 3, 'full']);
    $stmt->execute(['sales_actual_visit', 'sales', 1, 1, 3, 'full']);
    $stmt->execute(['sales_referral', 'sales', 1, 1, 3, 'full']);
    ...
    $stmt->execute(['coach_actual_comm', 'coach', 1, 1, 3, 'full']);
    $stmt->execute(['coach_referral_visit', 'coach', 1, 1, 3, 'full']);
    $stmt->execute(['coach_referral', 'coach', 1, 1, 3, 'full']);
}
