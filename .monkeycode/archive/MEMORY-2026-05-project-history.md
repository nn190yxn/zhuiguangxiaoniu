...
[学习中心复盘中心产品口径]
- Date: 2026-05-13
- Context: 用户要求在学习中心增加“复盘”接口，用于上传录音并生成评价报告
- Instructions:
  - 学习中心需要增加“复盘”入口，包含销售复盘、续费复盘、体测解读复盘三个入口。
  - 员工端直接上传录音文件，重点支持 m4a 类型，同时可兼容常见音频格式。
  - 后台需要调用录音转文字服务，再调用 AI 生成对应评价报告。
  - 录音转文字优先按火山/豆包语音识别适配，密钥和真实 endpoint 后续放在服务器环境变量，不能写入代码仓库或前端。
  - 评价报告应支持员工本人查看和下载。
  - 复盘记录和报告必须保留在服务器，后台管理端也能查看。
...
[线上内网持续可用要求]
- Date: 2026-05-11
- Context: 用户说明当前内网已有很多同事在使用，修本地并同步线上时必须确保线上可同步使用
- Instructions:
  - 后续任何会同步到 `/www/wwwroot/122.51.223.46/` 的修改都按生产变更处理，必须优先保证线上内网持续可用。
  - 同步线上前必须先确认文件是否为正在使用的入口或接口；若是，必须先备份、做最小变更、语法/静态检查通过后再覆盖。
  - 覆盖线上后必须立即做低风险验证，例如 HTTP 200、关键脚本路径存在、真实账号只读接口或无副作用参数校验。
  - 不得一次性批量覆盖多个正在使用的页面或接口；页面、接口、脚本应分批同步，出现异常可快速定位和回滚。
  - 不确定是否影响线上使用时，应先保留本地修改并向用户确认，不直接上线。
...
- Instructions:
  ...
  - 所有带顶部 `.site-header .topbar .nav` 的页面，统一由 `/internal-auth.js` 自动重写为同一套导航标签与顺序（工作台/表格/制度/知识/学习/培训/体测/教案/问卷/我的，管理角色追加“管理员”）。
