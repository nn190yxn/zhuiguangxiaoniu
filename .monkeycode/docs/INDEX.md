...
小程序请求与上传现已收口到 `real_sync/mini-program/utils/api.js`，统一请求 ID、15/60 秒超时、错误分类、设备会话刷新队列、幂等键、状态版本、上传 SHA-256、进度和失败恢复。导航工具统一 Tab 与普通页语义，能力白名单按最低客户端版本控制首页入口。`app.json` 当前登记 18 个发布页面，产品范围为演练、动作知识库、工作量和认证及员工身份基础支撑，底部导航固定为工作量、演练和我的。统一发布预检的 `miniprogram_core_scope` 阶段执行页面静态契约、路由与发布检查、知识库阅读、演练和工作量测试，定向回归为 25/25，预检配置回归为 5/5；历史十域矩阵与范围外页面继续保留在仓库中。
...
全站发布预检由 `real_sync/scripts/platform_regression_preflight.mjs` 统一编排。机器配置将波次 0 至 6 映射到 89 项覆盖、全量 Node/PHP、迁移、平台、权限、同步、文件、任务、AI、历史入口、小程序核心范围、文档链接和补丁格式共 17 个阶段；本地失败保持非零退出，数据库环境边界与生产审批分别输出 `blocked_external` 和 `approval_required`。小程序阶段现以三核心模块和基础支撑页面为发布口径。数据库 readiness dry-run、微信开发者工具自动化和真机旅程仍属于外部环境门禁，生产发布继续要求人工审批。
...
| [招聘简历导出](./模块/招聘简历导出.md) | 招聘简历导出数据源、权限筛选、排序和 XLSX 改造状态 |
