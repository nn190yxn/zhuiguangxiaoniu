...
将 `pages/index/index` 规划为登录后的“今日工作台”。第一期通过现有认证状态、公司后台统一发布的待办接口和统一导航组成可恢复的首页状态，首屏最多展示五条今日待办，底部统一提供四个核心 Tab。
...
- 展示范围：第一期最多展示五条；首页按服务端统一待办转换展示类型，使用待办自身的 `route`、`action_text` 和截止时间字段。
- 排序规则：优先沿用服务端返回顺序，并在接口契约中明确排序口径。
- 错误处理：通过 `view-state.js` 生成错误或离线状态，并提供 `retryHome`。

### 导航

- 工作量、演练、数据中心、我的使用 `navigation.open` 并由工具识别 Tab 页面。
...
- 数据转换：验证最多五条、优先级、类型名称、摘要数量和稳定路由映射。
