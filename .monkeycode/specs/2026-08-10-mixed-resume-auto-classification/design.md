...
系统增加混合简历批次。招聘人员上传前无需选择单个岗位，系统保存所有文件后，根据文件名、简历识别结果和候选岗位规则生成分类结果。高置信度结果自动进入岗位队列，低置信度结果进入人工确认队列。
...
- 读取批次创建时冻结的候选岗位集合。
- 组合文件名信号、结构化字段、关键词证据和硬性条件证据。
- 生成排序后的候选岗位列表。
- 根据阈值写入高置信度、待确认或未归类状态。
- 保存分类版本和可回放证据。
...
3. 自动分类结果必须关联至少一个岗位规则或文件名证据；规则未发布的需求只产生待分类状态。
4. 待确认和未归类简历不会自动进入岗位预约队列。
...
- 服务测试：候选岗位集合冻结、单岗位分类、多岗位相似结果、无法分类和人工确认。
