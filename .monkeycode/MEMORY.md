...
[知识卡二期任务9员工端阅读化]
- Date: 2026-08-26
- Context: Agent 在执行 1417 张知识卡二期员工端全量检索阅读改造时确认
- Category: 代码模式
- Instructions:
  - 员工端知识库小程序和移动 H5 统一按“阅读知识库”设计，不再把完成学习、学习进度和知识积分作为主流程入口。
  - 列表、详情、收藏和最近浏览只展示 `status=1 AND publication_status='published'` 内容；下架历史个人记录可保留但不得泄露内容元数据。
  - 详情页展示版本时间、内容类型、领域、难度、风险和来源摘要；原始 Markdown、source_path、source_snapshot_json 不作为员工端渲染输入。
...
[沟通方式偏好]
- Date: 2026-08-07（2026-09-16 再次强调）
- Context: 用户要求后续说明使用易懂表达；2026-09-16 用户明确要求"说中文"
- Instructions:
  - 后续回复和内部推理都使用中文，不要用英文作答。
  - 表达要简单、直接、少术语。
  - 出现必要技术术语时，同时用简短文字解释含义。

[开发推进时持续对照企业微信总计划]
- Date: 2026-06-23
- Context: 用户要求继续开发时及时关注整个开发计划的进度
- Instructions:
  - 每一轮企业微信相关开发都要先对照总计划判断当前所属阶段、已完成项和剩余缺口。
  - 推进局部功能时要同步关注第一阶段收口、第二阶段消息闭环和后续阶段边界，避免偏离主计划。
  - 重要阶段进展应及时写回计划文档，作为后续开发和联调基线。

[平板展示版采用包豪斯几何方向]
- Date: 2026-06-20
- Context: 用户在平板展示版视觉升级阶段确认参考 huashu-design 风格库并选定方向
- Instructions:
  - 平板展示版优先采用包豪斯几何风格，强化圆、方、斜切条和模块化几何分区。
  - 版面需要降低文字拥挤感，放大标题字号，拉开行高和留白，增强远距可读性。
  - 配色以米白底、品牌橙蓝为主，辅以少量高亮黄作为强调色。
  - 页面要保持家长可读和门店展示适配，设计感通过几何构成和排版节奏体现。

[对外展示页优先做短页系列]
- Date: 2026-06-20
- Context: 用户在规划公众号对外展示层时补充截图使用习惯
- Instructions:
  - 对外展示用的 HTML 页面优先控制为短页，一页只讲一个重点，方便手机整屏截图。
  - 优先采用“多做几张短页”而不是“做一张很长的长页”的方式组织展示内容。
  - 后续展示层设计需要兼顾截图传播场景，优先服务公众号配图和手机端浏览效果。

[对外展示页文案要家长化且去说明区]
- Date: 2026-06-20
- Context: 用户在查看样板页文案时补充外宣表达要求
- Instructions:
  - 对外展示页的文案要用家长容易看懂的表达，减少官方、书面、内部化口吻。
  - 页面内不要保留“这张适合做什么”“短页更适合截图”等说明性文字。
  - 展示页最终成品只保留面向外部读者的正文内容，不保留给内部协作看的解释区。

[修复与功能增设必须先制定完整计划]
- Date: 2026-07-24
- Context: 用户要求 Agent 以 10 年以上经验的资深全站架构师标准执行后续任务，并再次强调持续主动完成剩余任务
- Instructions:
...
[现网专项审查统一台账与修复顺序口径]
- Date: 2026-07-24
- Context: 用户在现网全面审计和工作量功能优化过程中再次明确执行方式
- Instructions:
  ...
  - 功能优化先完成问题发现和需求沟通，经过用户逐项确认后再编写正式开发文档并逐项实施。
...
- Instructions:
  ...
  - 2026-09-18 事实：该服务器开启了腾讯云主机安全的“密码破解”暴破防护，会收到常态化 SSH 自动扫描告警（示例来源 `123.56.157.253`，阿里云北京 AS37963，1 次尝试即被自动阻断）。此类告警多为公网扫描噪音，**不要默认等同于 Agent 行为**：Agent 使用密钥认证、不会触发密码破解规则，且沙箱出口为台湾/香港代理 ASN，与告警来源 IP 不同。
  - 重要关联：主机安全暴破防护在主机网络层按来源 IP 丢包，被拉黑的 IP 到该机器**所有端口**都会超时。因此“目标主机全端口 5 秒超时”也可能来自该机制拉黑了沙箱代理池出口。排查时应在腾讯云主机安全控制台核对拦截列表是否含 `103.156.242.x`、`188.253.127.x`、`114.37.226.x` 等沙箱出口网段，若有则解封后即可恢复。
  - 安全加固方向（需用户执行，Agent 不修改系统服务配置）：`sshd_config` 设 `PasswordAuthentication no`、`PermitRootLogin prohibit-password`；安全组 SSH 仅放行常用来源 IP，不对全网开放 22/2222；确保 root 密码为强随机值。
...
- Instructions:
  ...
  - 当前迭代优先完善桌面和手机 H5 Web 应用功能，小程序开发安排在该 Web 应用工作完成后。
