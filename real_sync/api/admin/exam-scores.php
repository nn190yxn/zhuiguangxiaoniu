...
$countStmt = $db->prepare("SELECT COUNT(*) FROM exam_records r LEFT JOIN staffs s ON r.user_id = s.user_id LEFT JOIN exams e ON r.module_id = e.id $whereSql");
...
$stmt = $db->prepare("SELECT r.*, s.name, s.phone, s.role, e.title as exam_title
    ...
    LEFT JOIN exams e ON r.module_id = e.id
    $whereSql
...
foreach ($rows as &$row) {
    ...
    if (empty($row['name']) && !empty($row['answers'])) {
        $decoded = json_decode($row['answers'], true);
        if (is_array($decoded)) {
            $row['name'] = $decoded['__meta']['staff_name'] ?? $decoded['staff_name'] ?? $decoded['name'] ?? '';
            $row['phone'] = $decoded['__meta']['phone'] ?? $decoded['phone'] ?? '';
        }
    }
    }
