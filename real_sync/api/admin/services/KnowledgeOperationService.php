...
class KnowledgeOperationService {
    ...
    public function listItems(array $filters = []): array {
        $where = [];
        $params = [];
        if (!empty($filters['publication_status'])) { $where[] = 'k.publication_status=:publication_status'; $params['publication_status'] = $filters['publication_status']; }
        if (!empty($filters['content_type'])) { $where[] = 'k.content_type=:content_type'; $params['content_type'] = $filters['content_type']; }
        if (!empty($filters['domain_code'])) { $where[] = 'k.domain_code=:domain_code'; $params['domain_code'] = $filters['domain_code']; }
        $limit = max(1, min(100, (int)($filters['limit'] ?? 50)));
        $params['limit'] = $limit;
        return $this->query('SELECT k.id,k.item_code,k.title,k.content_type,k.domain_code,k.risk_level,k.publication_status,k.current_version_id,k.source_batch_id,k.updated_at FROM knowledge_items k'.($where ? ' WHERE '.implode(' AND ', $where) : '').' ORDER BY k.updated_at DESC LIMIT :limit', $params);
    }
    ...
    public function createRelation(int $sourceId, int $targetId, string $type, string $note, array $actor): void {
        $note = trim($note);
        if ($sourceId === $targetId || !in_array($type, ['candidate', 'merged', 'kept_separate', 'rejected'], true) || $note === '') throw new InvalidArgumentException('关系参数无效');
        $stmt = $this->db->prepare('INSERT INTO knowledge_item_relations (source_item_id,target_item_id,relation_type,reviewed_by,reviewed_at,review_note) VALUES (?,?,?,?,NOW(),?)');
        $stmt->execute([$sourceId, $targetId, $type, $actor['staff_id'] ?? $actor['user_id'] ?? null, $note]);
        $this->recordAudit($actor, 'create_relation', 'relation', (string)$this->db->lastInsertId(), [], ['source_item_id'=>$sourceId,'target_item_id'=>$targetId,'relation_type'=>$type], $note);
    }
