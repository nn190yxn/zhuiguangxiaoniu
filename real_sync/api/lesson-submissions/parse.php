...
require_once __DIR__ . '/LessonWorkbookParser.php';
require_once __DIR__ . '/LessonWordParser.php';
require_once __DIR__ . '/LessonSubmissionService.php';

header('Content-Type: application/json; charset=utf-8');
handleCORS();
$context = platformApiContext(['domain' => 'lesson_review', 'action' => 'lesson_submission.parse']);
...
$result = (new LessonSubmissionService(getDB(), new PlatformPrivateFileStorage()))->parseUploadedFile(
    (int) ($input['submission_id'] ?? 0),
    (int) ($input['source_file_id'] ?? 0),
    (int) $auth->staffId()
);
...
$logger->log('info', 'lesson_submission.parse', $context, ['submission_id' => $result['submission_id'], 'source_file_id' => $result['source_file_id'], 'status' => $result['status']]);
