...
try {
    $result = drillV2RunIdempotent($pdo, $context, 'drill.media.recover_transcription', $input, function () use ($pdo, $context, $input): array {
        $service = new DrillMediaService($pdo);
        return $service->recoverTranscription(
    ...
    error_log('Drill v2 audio recovery failed: ' . $exception->getMessage());
    drillV2Error(500, '音频转写恢复失败', [], 500);
}
