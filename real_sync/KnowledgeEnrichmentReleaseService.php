    ...
    public function publish(array $input, array $actor): array
    {
        ...
        try {
            ...
            foreach ($rows as $row) {
                ...
                $hash = hash('sha256', (string)$row['enriched_content_json']);
                if (!hash_equals($hash, (string)$row['enriched_content_sha256'])) throw new RuntimeException('增强稿哈希校验失败');
                $content = json_decode((string)$row['enriched_content_json'], true);
            ...
            $this->audit($actor, 'enrichment_publish', $ids, ['release_batch_id' => $batch]);
            $this->db->commit();
