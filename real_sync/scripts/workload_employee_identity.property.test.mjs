...
const legalRoles = new Set(['sales', 'coach', 'manager', 'teaching_supervisor', 'supervisor']);
...
test('property 27: authorized role selection is bounded by the legal role options', () => {
  ...
  for (const index of generatedIndexes) {
    const roleOptions = ['sales', 'coach', 'manager', 'teaching_supervisor', 'supervisor'];
    const selected = roleOptions[index];
    assert.equal(selected === undefined || legalRoles.has(selected), true);
  }
  assert.match(miniJs, /roleOptions:\s*\[\{ label: '销售', value: 'sales' \}, \{ label: '教练', value: 'coach' \}, \{ label: '店长', value: 'manager' \}, \{ label: '教学主管', value: 'teaching_supervisor' \}, \{ label: '督导', value: 'supervisor' \}\]/);
