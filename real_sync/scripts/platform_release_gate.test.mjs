...
function healthyObservation(overrides = {}) {
  return {
    ...
    metrics: {
      ...
      data_differences: 0,
      shadow_differences: { count: 0, critical_count: 0, rate: 0, categories: [] },
      queue_oldest_minutes: 1,
      media_queue: { pending: 0, oldest_minutes: 0, failed: 0 },
...
test('全部发布阈值生成稳定停止原因', () => {
  const result = evaluateReleaseObservation(healthyObservation({
    metrics: {
      ...
      data_differences: 1,
      shadow_differences: { count: 3, critical_count: 1, rate: 0.02, categories: ['field_structure'] },
      queue_oldest_minutes: 16,
      media_queue: { pending: 50, oldest_minutes: 16, failed: 1 },
