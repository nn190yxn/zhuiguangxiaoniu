...
echo "Draft OK. ID: " . $res['data']['report_id'] . "\n";
...
if ($res2['code'] === 400 && strpos($res2['message'], '凭证') !== false) {
    echo "PASS: Submission blocked correctly for missing evidence.\n";
} else {
    echo "FAIL: Submission should have been blocked for missing evidence.\n";
}
...
if (count($tasks) > 0) {
    ...
    if ($tasks2[0]['audit_status'] === 'approved') {
        echo "PASS: Full audit flow completed successfully.\n";
    } else {
        echo "FAIL: Audit status not updated correctly.\n";
    }
} else {
    echo "FAIL: No tasks generated after submission with evidence.\n";
}
